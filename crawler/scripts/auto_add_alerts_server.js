...
const searchCoin = async(page, context, strategyName, symbol) => {
  ...
  searchBtn[0].click()
    ...
    // 点击下拉选择框，选择 "alert() function calls only"
    console.log('点击下拉选择框...');
    await page.waitForSelector('span[data-qa-id="ui-kit-disclosure-control operator-select"]', { state: 'visible' });
    await page.click('span[data-qa-id="ui-kit-disclosure-control operator-select"]');
    
    await wait(page, 1000)
    
    // 选择 "alert() function calls only" 选项
    console.log('选择 alert() function calls only 选项...');
    await page.waitForSelector('div#id_item_strategy_alerts[role="option"]', { state: 'visible' });
    await page.click('div#id_item_strategy_alerts[role="option"]');
    ...
    // 清空输入框并输入新的名称
    await page.fill(alertNameSelector, '');
    await wait(page, 500);
...
const initPage = async () => {
  ...
  const context = contexts[0]; // 取第一个上下文，通常是默认上下文
    ...
    await addAlertsForSymbols(page, context, symbols, '小币种防爆v5.5', 0);
